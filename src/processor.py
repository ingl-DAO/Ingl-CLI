...
async def ingl_init(payer_keypair: KeypairInput, validator_pubkey: PubkeyInput, init_commission: int, max_primary_stake: int, nft_holders_share: int, initial_redemption_fee: int, is_validator_id_switchable: bool, unit_backing: int, redemption_fee_duration: int, proposal_quorum: int, creator_royalties: int, governance_expiration_time: int, rarities: List[int], rarity_names: List[str], twitter_handle: str, discord_invite: str, validator_name: str, collection_uri: str, website: str, default_uri: str, client: AsyncClient, log_level: int = 0) -> str:
    ...
    collection_holder_pubkey, _collection_holder_pubkey_bump = Pubkey.find_program_address(
    ...
    metadata_pda, _metadata_pda_bump = Pubkey.find_program_address(
        [b"metadata", bytes(metaplex_program_id), bytes(mint_pubkey)], metaplex_program_id)
    master_edition_pda, _master_edition_bump = Pubkey.find_program_address(
        [b"metadata", bytes(metaplex_program_id), bytes(mint_pubkey), b"edition"], metaplex_program_id)
    ...
    registry_program_config_key, _registry_program_config_bump = Pubkey.find_program_address(
    ...
    registry_config_data = RegistryConfig.parse(
        registry_config_account.value.data)
    ...
    storage_key, _storage_bump = Pubkey.find_program_address(
        [b'storage', storage_numeration.to_bytes(4, "big")], ingl_constants.REGISTRY_PROGRAM_ID)
    ...
    mint_associated_meta = AccountMeta(
        mint_associated_account_pubkey, False, True)
    spl_program_meta = AccountMeta(
        spl_constants.TOKEN_PROGRAM_ID, False, False)
    ...
    this_program_data_meta = AccountMeta(
        this_program_data_pubkey, False, False)
    ...
    registry_program_config_meta = AccountMeta(
        registry_program_config_key, False, True)
    program_meta = AccountMeta(get_program_id(), False, False)
    team_account_meta = AccountMeta(
        ingl_constants.TEAM_ACCOUNT_KEY, False, True)
    ...
    # print(data)
    transaction.add(ComputeBudgetInstruction().set_compute_unit_limit(
        300_000, payer_keypair.pubkey))
    transaction.add(Instruction(accounts=accounts,
                    program_id=get_program_id(), data=data))
...
async def mint_nft(payer_keypair: KeypairInput, mint_keypair: KeypairInput, client: AsyncClient, log_level: int = 0) -> str:
    ...
    metaplex_program_id = Pubkey.from_string(
        "metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s")
    metadata_pda, _metadata_pda_bump = Pubkey.find_program_address([b"metadata", bytes(
        metaplex_program_id), bytes(mint_keypair.pubkey)], metaplex_program_id)
    collection_master_edition_pda, _master_edition_bump = Pubkey.find_program_address(
        [b"metadata", bytes(metaplex_program_id), bytes(collection_mint_pubkey), b"edition"], metaplex_program_id)
    mint_edition_pda, _mint_edition_bump = Pubkey.find_program_address([b"metadata", bytes(
        metaplex_program_id), bytes(mint_keypair.pubkey), b"edition"], metaplex_program_id)
    collection_account_pda, _collection_account_bump = Pubkey.find_program_address(
        [b"metadata", bytes(metaplex_program_id), bytes(collection_mint_pubkey)], metaplex_program_id)
    nft_account_pubkey, _nft_account_bump = Pubkey.find_program_address([bytes(
        ingl_constants.NFT_ACCOUNT_CONST, 'UTF-8'), bytes(mint_keypair.pubkey)], get_program_id())
    ingl_config_pubkey, _ingl_config_bump = Pubkey.find_program_address(
        [bytes(ingl_constants.INGL_CONFIG_SEED, 'UTF-8')], get_program_id())
    ...
    instruction_data = build_instruction(InstructionEnum.enum.MintNft(
        switchboard_state_bump=0, permission_bump=0, log_level=log_level))
    transaction = Transaction()
    transaction.add(ComputeBudgetInstruction().set_compute_unit_limit(
        400_000, payer_keypair.pubkey))
    transaction.add(Instruction(accounts=accounts,
                    program_id=get_program_id(), data=instruction_data))
    try:
...
async def delegate_nft(payer_keypair: KeypairInput, mint_pubkey: PubkeyInput, client: AsyncClient, log_level: int = 0) -> str:
    nft_account_pubkey, _nft_account_bump = Pubkey.find_program_address([bytes(
        ingl_constants.NFT_ACCOUNT_CONST, 'UTF-8'), bytes(mint_pubkey.pubkey)], get_program_id())
    mint_associated_account_pubkey = assoc_instructions.get_associated_token_address(
    ...
    nft_account_meta = AccountMeta(nft_account_pubkey, False, True)
    mint_associated_meta = AccountMeta(
        mint_associated_account_pubkey, False, False)
...
async def undelegate_nft(payer_keypair: KeypairInput, mint_pubkey: PubkeyInput, client: AsyncClient, log_level: int = 0) -> str:
    ...
    mint_associated_account_pubkey = assoc_instructions.get_associated_token_address(
        payer_keypair.pubkey, mint_pubkey.pubkey)
    authorized_withdrawer_key, _authorized_withdrawer_bump = Pubkey.find_program_address(
    ...
    authorized_withdrawer_meta = AccountMeta(
        authorized_withdrawer_key, False, False)
    system_program_meta = AccountMeta(system_program.ID, False, False)

    general_account_info = await client.get_account_info(general_account_pubkey)
    general_data = GeneralData.parse(
        general_account_info.value.data)

    nft_account_info = await client.get_account_info(nft_account_pubkey)
    nft_data = NftData.parse(nft_account_info.value.data)

    interested_proposals = [
        proposal for proposal in general_data.unfinalized_proposals if proposal in nft_data.all_votes]
    interested_proposals_account_metas = [AccountMeta(Pubkey.find_program_address([bytes(
        ingl_constants.INGL_PROPOSAL_KEY, 'UTF-8'), (proposal).to_bytes(4, "big")], get_program_id())[0], False, True) for proposal in interested_proposals]
    ...
    # print(accounts)
    instruction_data = build_instruction(
        InstructionEnum.enum.UnDelegateNFT(log_level=log_level, ))
    transaction = Transaction()
    transaction.add(Instruction(accounts=accounts,
                    program_id=get_program_id(), data=instruction_data))
    try:
...
async def create_vote_account(validator_keypair: KeypairInput, client: AsyncClient, log_level: int = 0) -> str:
    ...
    vote_program_meta = AccountMeta(
        ingl_constants.VOTE_PROGRAM_ID, False, False)
    spl_program_meta = AccountMeta(
        spl_constants.TOKEN_PROGRAM_ID, False, False)
    stake_account_meta = AccountMeta(expected_stake_key, False, True)
    stake_program_meta = AccountMeta(
        ingl_constants.STAKE_PROGRAM_ID, False, False)
    ...
    data = InstructionEnum.build(
        InstructionEnum.enum.CreateVoteAccount(log_level=log_level, ))
    transaction = Transaction()
    transaction.add(Instruction(accounts=accounts,
                    program_id=get_program_id(), data=data))
    try:
...
async def init_rebalance(payer_keypair: KeypairInput, client: AsyncClient, log_level: int = 0) -> str:
    ...
    # print(accounts)
    data = InstructionEnum.build(
        InstructionEnum.enum.InitRebalance(log_level=log_level, ))
    transaction = Transaction()
    transaction.add(Instruction(accounts=accounts,
                    program_id=get_program_id(), data=data))
    try:
...
async def finalize_rebalance(payer_keypair: KeypairInput, client: AsyncClient, log_level: int = 0) -> str:
    ...
    stake_program_meta = AccountMeta(
        ingl_constants.STAKE_PROGRAM_ID, False, False)
    sysvar_stake_history_meta = AccountMeta(
        solders.sysvar.STAKE_HISTORY, False, False)
    general_account_meta = AccountMeta(general_account_pubkey, False, True)
    config_account_meta = AccountMeta(config_account_pubkey, False, False)
    ...
    data = InstructionEnum.build(
        InstructionEnum.enum.FinalizeRebalance(log_level=log_level, ))
    transaction = Transaction()
    transaction.add(Instruction(accounts=accounts,
                    program_id=get_program_id(), data=data))
    try:
...
async def process_rewards(payer_keypair: KeypairInput, client: AsyncClient, log_level: int = 0) -> str:
    ...
    authorized_withdrawer_meta = AccountMeta(
        authorized_withdrawer_key, False, True)
    config_account_meta = AccountMeta(config_account_pubkey, False, False)
    general_account_meta = AccountMeta(general_account_pubkey, False, True)
    ...
    config_data = InstructionEnum.build(
        InstructionEnum.enum.ProcessRewards(log_level=log_level, ))
    transaction = Transaction()
    transaction.add(Instruction(accounts=accounts,
                    program_id=get_program_id(), data=config_data))
    try:
        t_dets = await sign_and_send_tx(transaction, client, payer_keypair)
        await client.confirm_transaction(tx_sig=t_dets.value, commitment="finalized", sleep_seconds=0.4, last_valid_block_height=None)
        return f"Transaction Id: [link=https://explorer.solana.com/tx/{str(t_dets.value)+get_explorer_suffix(get_network())}]{str(t_dets.value)}[/link]"
...
async def nft_withdraw(payer_keypair: KeypairInput, mints: List[Pubkey], client: AsyncClient, log_level: int = 0) -> str:
    authorized_withdrawer_key, _authorized_withdrawer_bump = Pubkey.find_program_address([bytes(
        ingl_constants.AUTHORIZED_WITHDRAWER_KEY, 'UTF-8'), bytes(vote_account_id.pubkey)], get_program_id())
    general_account_pubkey, _general_account_bump = Pubkey.find_program_address(
        [bytes(ingl_constants.GENERAL_ACCOUNT_SEED, 'UTF-8')], get_program_id())
    config_account_pubkey, _config_account_bump = Pubkey.find_program_address(
        [bytes(ingl_constants.INGL_CONFIG_SEED, 'UTF-8')], get_program_id())
    ...
    for mint_pubkey in mints:
        ...
        gem_account_pubkey, _gem_account_bump = Pubkey.find_program_address([bytes(
            ingl_constants.GEM_ACCOUNT_CONST, 'UTF-8'), bytes(mint_pubkey.pubkey)], get_program_id())
        accounts.append(AccountMeta(gem_account_pubkey, False, True))

    accounts.append(sys_program_meta)
    # print(accounts)
    data = InstructionEnum.build(InstructionEnum.enum.NFTWithdraw(
        log_level=log_level, cnt=len(mints)))
    transaction = Transaction()
    transaction.add(Instruction(accounts=accounts,
                    program_id=get_program_id(), data=data))
    try:
...
async def init_governance(payer_keypair: KeypairInput, mint: PubkeyInput, client: AsyncClient, title: str, description: str, governance_type: Optional[GovernanceType.enum] = None, config_account_type: Optional[ConfigAccountType.enum] = None, vote_account_governance: Optional[VoteAccountGovernance.enum] = None,  log_level: int = 0) -> str:
    ...
    nft_account_data_pubkey, _nft_account_data_bump = Pubkey.find_program_address(
        [bytes(ingl_constants.NFT_ACCOUNT_CONST, 'UTF-8'), bytes(mint.pubkey)], get_program_id())
    ...
    proposal_numeration = GeneralData.parse(
        data.value.data).proposal_numeration
    print(f"proposal_numeration: {proposal_numeration}")

    config_data = ValidatorConfig.parse((await client.get_account_info(config_account_pubkey)).value.data)
    vote_account_key = Pubkey(config_data.vote_account)

    proposal_pubkey, _proposal_bump = Pubkey.find_program_address([bytes(
        ingl_constants.INGL_PROPOSAL_KEY, 'UTF-8'), (proposal_numeration).to_bytes(4, "big")], get_program_id())
    ...
    instruction_data = build_instruction(InstructionEnum.enum.InitGovernance(),  title=title, description=description, governance_type=governance_type,
                                         config_account_type=config_account_type, vote_account_governance=vote_account_governance, log_level=log_level)
...
async def vote_governance(payer_keypair: KeypairInput, vote: Bool, proposal_numeration: int, mints: List[Pubkey], client: AsyncClient, log_level: int = 0) -> str:
    proposal_pubkey, _proposal_account_bump = Pubkey.find_program_address([bytes(
        ingl_constants.INGL_PROPOSAL_KEY, 'UTF-8'), (proposal_numeration).to_bytes(4, "big")], get_program_id())
    ...
    for mint in mints:
        ...
        nft_account_pubkey, _nft_account_bump = Pubkey.find_program_address(
            [bytes(ingl_constants.NFT_ACCOUNT_CONST, 'UTF-8'), bytes(mint)], get_program_id())
    ...
    # print(accounts)
    instruction_data = build_instruction(InstructionEnum.enum.VoteGovernance(
        log_level=log_level, numeration=proposal_numeration, vote=vote, cnt=len(mints)))
    transaction = Transaction()
    transaction.add(Instruction(accounts=accounts,
                    program_id=get_program_id(), data=instruction_data))
    try:
...
async def finalize_governance(payer_keypair: KeypairInput, proposal_numeration: int, client: AsyncClient, log_level: int = 0) -> str:
    proposal_account_key, _proposal_account_bump = Pubkey.find_program_address([bytes(
        ingl_constants.INGL_PROPOSAL_KEY, 'UTF-8'), (proposal_numeration)], get_program_id())
    config_account_key, _config_account_bump = Pubkey.find_program_address(
        [bytes(ingl_constants.INGL_CONFIG_KEY, 'UTF-8')], get_program_id())
    general_account_key, _general_account_bump = Pubkey.find_program_address(
        [bytes(ingl_constants.INGL_GENERAL_KEY, 'UTF-8')], get_program_id())
    ...
    # print(accounts)
    instruction_data = build_instruction(InstructionEnum.enum.FinalizeGovernance(
...
async def execute_governance(payer_keypair: KeypairInput, proposal_numeration: int, client: AsyncClient, log_level: int = 0) -> str:
    ...
    if governance_type == 0:
        ...
        programdata_key, _programdata_bump = Pubkey.find_program_address(
            [bytes(get_program_id())], ingl_constants.BPF_LOADER_UPGRADEABLE)
        upgrade_authority_key, _upgrade_authority_bump = Pubkey.find_program_address([bytes(
            ingl_constants.INGL_PROGRAM_AUTHORITY_KEY, 'UTF-8'), bytes(get_program_id())], get_program_id())
        ...
        vote_account_key, _vote_account_bump = Pubkey.find_program_address(
            [bytes(ingl_constants.INGL_VOTE_KEY, 'UTF-8'),], get_program_id())
        authorized_withdrawer_key, _authorized_withdrawer_bump = Pubkey.find_program_address(
            [bytes(ingl_constants.INGL_VOTE_AUTHORIZED_WITHDRAWER_KEY, 'UTF-8'),], get_program_id())
    ...
    # print(accounts)
    instruction_data = build_instruction(InstructionEnum.enum.ExecuteGovernance(
        log_level=log_level, numeration=proposal_numeration))
    transaction = Transaction()
    transaction.add(Instruction(accounts=accounts,
                    program_id=get_program_id(), data=instruction_data))
...
def upload_uris(payer_keypair: KeypairInput, uris: List[str], rarity: int, client: Client, log_level: int = 0) -> str:
    ...
    t_dets = None
    try:
        instruction_data = build_instruction(InstructionEnum.enum.UploadUris(
...
async def register_program(payer_keypair: KeypairInput, program_key: Pubkey, client: AsyncClient,) -> str:
    ...
    team_account_meta = AccountMeta(
        pubkey=ingl_constants.TEAM_ACCOUNT_KEY, is_signer=False, is_writable=True)
    system_program_meta = AccountMeta(
        pubkey=system_program.ID, is_signer=False, is_writable=False)
...
async def fractionalize_existing(payer_keypair: KeypairInput, authorized_withdrawer: KeypairInput, vote_account_pubkey: PubkeyInput, validator_pubkey: PubkeyInput, init_commission: int, max_primary_stake: int, nft_holders_share: int, initial_redemption_fee: int, is_validator_id_switchable: bool, unit_backing: int, redemption_fee_duration: int, proposal_quorum: int, creator_royalties: int, governance_expiration_time: int, rarities: List[int], rarity_names: List[str], twitter_handle: str, discord_invite: str, validator_name: str, collection_uri: str, website: str, default_uri: str, client: AsyncClient, log_level: int = 0) -> str:
    ...
    pda_authorized_withdrawer, _pda_authorized_withdrawer_bump = Pubkey.find_program_address(
        [bytes(ingl_constants.INGL_AUTHORIZED_WITHDRAWER_KEY, 'UTF-8')], get_program_id())
...
async def inject_testing_data(payer_keypair: KeypairInput, mints: List[Pubkey], client: AsyncClient, log_level: int = 0) -> str:
    ...
    for mint_pubkey in mints:
        ...
        nft_account_pubkey, _nft_account_bump = Pubkey.find_program_address([bytes(
            ingl_constants.NFT_ACCOUNT_CONST, 'UTF-8'), bytes(mint_pubkey)], get_program_id())
        accounts.append(AccountMeta(nft_account_pubkey, False, True))

    accounts.append(sys_program_meta)
    # print(accounts)
    data = InstructionEnum.build(InstructionEnum.enum.InjectTestingData(
