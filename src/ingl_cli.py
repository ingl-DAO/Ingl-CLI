...
async def ingl(keypair, validator, authorized_withdrawer, vote_account, log_level):
    ...
    while init_commission > 100 or init_commission < 0:
        ...
        init_commission = click.prompt(
            "Enter the Commission(%) to be set for the validator ( > 0, <=100)", type=int)
    max_primary_stake = 1
    counter = 0
    while max_primary_stake < 1_030_000_000:
        ...
        max_primary_stake = click.prompt(
            "Enter the maximum primary stake(in lamports) to be set for the validator (>1,030,000,000)", type=int)
    ...
    while nft_holders_share > 100 or nft_holders_share < 50:
        ...
        nft_holders_share = click.prompt(
            "Enter the NFT Holders Share(%) to be set for the validator (>50, <=100)", type=int)
    ...
    while initial_redemption_fee > 25 or initial_redemption_fee < 0:
        ...
        initial_redemption_fee = click.prompt(
            "Enter the Initial Redemption Fee(%) to be set for the validator (>=0, <=25)", type=int)
    ...
    while unit_backing < 1_030_000_000:
        ...
        unit_backing = click.prompt(
            "Enter the Unit Backing(in lamports) to be set for the validator (>1,030,000,000) ", type=int)
    ...
    while redemption_fee_duration > 63_072_000:
        ...
        redemption_fee_duration = click.prompt(
            "Enter the Redemption Fee duration(in seconds) to be set for the validator (<63,072,000)", type=int)
    proposal_quorum = 60
    counter = 0
    while proposal_quorum < 65 or proposal_quorum > 100:
        ...
        proposal_quorum = click.prompt(
            "Enter the Proposal Quorum(%) to be set for governance proposals (>=65, <=100) ", type=int)
    creator_royalty = 300
    counter = 0
    while creator_royalty > 200 or creator_royalty < 0:
        ...
        creator_royalty = click.prompt(
            "Enter the Creator Royalty(in basis points) to be set for the validator (>=0, <200)", type=int)
    ...
    while governance_expiration_time < 35 or governance_expiration_time > 365:
        ...
        governance_expiration_time = click.prompt(
            "How long should a governance take to expire? (in days) (>=35 days, <=365 days) ", type=int)
    governance_expiration_time = governance_expiration_time * 86_400
    twitter_handle = click.prompt(
        "Enter the Twitter handle of the validator: ", type=str)
    discord_invite = click.prompt(
        "Enter the Discord Invite of the validator: ", type=str)
    validator_name = click.prompt(
        "Enter the Name of the validator: ", type=str)
    website = click.prompt("Enter the Website of the validator: ", type=str)
    default_uri = click.prompt(
        "Enter the Default URI of the validator: ", type=str)
...
async def process_create_governance(keypair, mint_id, log_level):
    ...
    if numeration == 0:
        ...
        try:
            buffer_address = parse_pubkey_input(click.prompt(
                "Enter the buffer address: ", type=str)).pubkey
        except Exception as e:
...
async def process_upload_uris(keypair, json_path, log_level):
    ...
    for cnt, rarity in enumerate(uris):
        z = 0
        while z < len(rarity):
            txs.append(upload_uris(payer_keypair,
                       rarity[z:z+11], cnt, uasyncclient, log_level).value)
            z += 11
        # print(txs)
...
@click.command(name="register_program", help="Register a Program with the Governance Registry, Argument: Name(str), Options: --keypair/-k, --program_id/-p")
@click.option('--keypair', '-k', default=get_keypair_path(), help="Enter the path to the keypair that will be used to sign this transaction. Defaults to the set config keypair")
...
async def process_inject_test(keypair, num_mints, log_level):
    ...
    for i in range(num_mints):
        while True:
            try:
                mints.append(parse_pubkey_input(click.prompt(
                    f"Enter the Mint Address for Mint {i+1}", type=str)).pubkey)
                break
            except Exception as e:
